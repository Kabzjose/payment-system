    ...
    <div className="fixed inset-0 bg-[#0E1116]/40 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
      <div className="w-full sm:max-w-md bg-[#F7F5F0] rounded-t-xl sm:rounded-xl max-h-[85vh] overflow-y-auto">
        <div className="sticky top-0 bg-[#F7F5F0] border-b border-[#E5E2DA] px-5 py-4 flex items-center justify-between">
          <span className="font-mono text-[11px] tracking-[0.15em] text-[#8B8578] uppercase">
            Payment detail
          </span>
          <button
            onClick={onClose}
            className="text-[#8B8578] hover:text-[#0E1116] text-[20px] leading-none"
            aria-label="Close"
          >
            &times;
          </button>
        ...
        <div className="p-5">
          ...
          {error && !loading && (
            <div className="text-[13px] text-[#C9402E] bg-[#C9402E]/8 border border-[#C9402E]/20 rounded-md px-3 py-2 mb-4">
              {error}
            </div>
          )}
          ...
          {payment && (
            <>
              <div className="flex items-start justify-between mb-1">
                <span className="font-serif text-3xl text-[#0E1116] tabular-nums">
              ...
              <p className="font-mono text-[11px] text-[#8B8578] mb-6">{payment.id}</p>

              <div className="mb-6">
                <h3 className="font-mono text-[11px] tracking-[0.15em] text-[#8B8578] uppercase mb-2">
                  Transactions
                </h3>
                {payment.transactions.length === 0 ? (
                  <p className="text-[13px] text-[#8B8578]">No transactions recorded yet.</p>
                ) : (
                  <div className="space-y-2">
                    {payment.transactions.map((t) => (
                      <div
                        key={t.id}
                        className="flex items-center justify-between text-[13px] py-2 border-b border-[#E5E2DA] last:border-b-0"
                      >
                        <div>
                          <div className="font-medium text-[#0E1116]">{transactionLabel(t.type)}</div>
                          <div className="font-mono text-[11px] text-[#8B8578]">{formatDate(t.created_at)}</div>
                        </div>
                        <div className="flex items-center gap-3">
                          <StatusPill status={t.status} />
                          <span className="font-mono text-[13px] tabular-nums text-[#0E1116]">
                            {t.type !== "charge" ? "−" : ""}
